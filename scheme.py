...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    # All non-atomirc expressions are lists (combinations)
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if rest:
            operands=Pair.map(rest, lambda x: scheme_eval(x, env))
        else:
            operands=nil
        return scheme_apply(fn, operands, env)
        # # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        # BEGIN PROBLEM 10
        new_instance=Frame(self)
        if len(formals)!=len(vals):
            raise SchemeError("Number of parameters not equal to number of arguments given")
        else:
            while formals!=nil:
                new_instance.define(formals.first, vals.first)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError(type(t), "has occurred")
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        Frame.define(env, target, result)
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        result=LambdaProcedure(target.rest, expressions.rest, env)
        Frame.define(env, target.first, result)
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    check_form(clause.rest, 1, 1)
    check_formals(Pair(clause.first, nil))
    value=scheme_eval(clause.rest.first, env)
    symbol=clause.first
    new_frame=Frame.make_child_frame(env, Pair(symbol, nil), Pair(value, nil))
...
def do_define_macro(expressions, env):
    ...
    macro_new=MacroProcedure(formals, body, env)
    env.define(macro_name, macro_new)
    return macro_name
    # END Problem 20
